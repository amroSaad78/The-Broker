Param(
    ...
    [parameter(Mandatory=$false)][string]$appName="broker",
...
function Install-Chart  {
    ...
    if ($chart -ne "common" -or $customRegistry)  {       # common is ignored when no secret must be deployed        
...
if ($externalDns -eq "aks") {
    ...
    Write-Host "Getting DNS of AKS of AKS $aksName (in resource group $aksRg)..." -ForegroundColor Yellow
...
if ($clean) {
    ...
    helm delete $(helm ls -q --filter 'e[a-z]+') 
...
Write-Host "Begin The broker installation using Helm" -ForegroundColor Green

$infras = ("rabbitmq", "keystore-data")
$charts = ("common", "apartment-api","identity-api", "owners-api", "signalrhub", "webclientagg", "webmvc")
$gateways = ("webclientapigw", "webclientaggapigw")
...
if ($deployInfrastructure) {
    ...
    Write-Host "The broker infrastructure (rabbitmq, redis, ...) charts aren't installed (-deployCharts is false)" -ForegroundColor Yellow
}
...
if ($deployCharts) {
    ...
    Write-Host "The broker non-infrastructure charts aren't installed (-deployCharts is false)" -ForegroundColor Yellow
}

Write-Host "helm charts installed......" -ForegroundColor Yellow
